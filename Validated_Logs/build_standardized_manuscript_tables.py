...
Notes
-----
- The manuscript defines efficiency/gap using the per-row columns (eff_pct, gap_pct) and
  then averages those percentages across configs. This script matches that approach.
- Exp. Winner counts are computed at the config level:
    (scenario, allocator, scale, experiment)
  using the dataset's `winner` field.
"""
...
# Keep a stable ordering for table rows.
MODEL_ORDER = [
    "ORACLE",
    "CPURSUITNEURALUCB",
    "GNEURALUCB",
...
# The manuscript's per-testbed tables use a slightly different per-paper ordering.
PAPER_MODEL_ORDERS = {
...
def _load_dataset(path: Path) -> pd.DataFrame:
    ...
    # Normalize a few columns to avoid mixed types.
...
def _winner_counts_by_model(configs: pd.DataFrame) -> dict[str, int]:
    """Return winner counts keyed by UPPERCASE model code."""
    counts: dict[str, int] = {}
...
def _compute_all_allocs_rows(df_paper: pd.DataFrame, runs: int) -> tuple[int, list[ModelRowAllAllocs], list[str], list[str]]:
    ...
    # Best efficiency / wins among non-Oracle models.
...
def render_table_cross_testbed_standardized(df: pd.DataFrame, runs: int) -> str:
    ...
    lines: list[str] = []
    lines.append(r"% AUTO-GENERATED by Validated_Logs/build_standardized_manuscript_tables.py")
    ...
    for paper in papers_order:
        ...
        # Re-order rows per the manuscript pattern.
...
def render_table_external_default_standardized(df: pd.DataFrame, runs: int) -> str:
    ...
    for paper in papers_order:
        ...
        # Keep a stable row order (same as external slice in the manuscript).
...
def main() -> int:
    ...
    default_input = Path(__file__).resolve().parent / "Master_Dataset_papers-4000_2000.csv"
    parser.add_argument("--input", type=Path, default=default_input, help="Path to Master_Dataset_papers-4000_2000.csv")
    parser.add_argument("--runs", type=int, default=5, help="Run-count slice to use (default: 5)")
    args = parser.parse_args()

    df = _load_dataset(args.input)

    # Limit to the papers we table in the manuscript.
