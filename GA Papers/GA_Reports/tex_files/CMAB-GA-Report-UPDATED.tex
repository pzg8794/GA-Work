...
\large{Empirical Assessment of iCMAB Variants and Hybrid Integration Framework for Adversarial Quantum Environments}}
...
\begin{abstract}
This report presents a comprehensive empirical evaluation of contextual multi-armed bandit (CMAB) models specifically designed for quantum network routing under adversarial and stochastic conditions. Using multi-run experiment suites (3-run and 5-run) across scaled capacity regimes (1$\times$, 1.5$\times$, 2$\times$) and two evaluator regimes (T-type and Tb-type), we systematically evaluate five informed contextual MAB (iCMAB) algorithms: CPursuit, CEpsilonGreedy, CThompsonSampling, CEXP4, and CEpochGreedy. Experimental scenarios span five environment conditions (NONE baseline, STOCHASTIC, MARKOV, ADAPTIVE ATTACKS, ONLINE ADAPTIVE ATTACKS), with performance measured as percentage efficiency relative to an oracle route. Results show that CPursuit consistently anchors the top tier, achieving an overall average efficiency of approximately 90.4\% (with peaks near 96.9\% in favorable settings), while CEpsilonGreedy forms a strong second tier around 87.8\%. CThompsonSampling and CEXP4 form a mid-range performance tier (about 67.5\% and 69.9\%, respectively), and CEpochGreedy remains clearly suboptimal (about 37.7\%). Capacity scaling from 1$\times$ to 2$\times$ and switching between T-type and Tb-type regimes produce only small fluctuations ($\approx$1--1.5 percentage points) in the relative ordering of these models. These empirically grounded rankings provide the baseline selection layer for future hybrid CExpNeuralUCB + iCMAB integration and inform where predictive intelligence (iCMAB) can most effectively augment adversarially robust bandit architectures.
\end{abstract}
...
\item \success{Evaluator Regime Comparison}: Tb-type runs offer a small but consistent advantage (typically $+0.2$ to $+1.5$ percentage points) over T-type for CPursuit, CEpsilonGreedy, CThompsonSampling, and CEXP4, without changing the overall ranking.
...
Our research objectives center on empirical evaluation of informed contextual multi-armed bandit (iCMAB) algorithms, with particular emphasis on identifying the most robust performers for subsequent integration into hybrid architectures (e.g., CExpNeuralUCB + iCMAB). The evaluation in this report focuses on the iCMAB subset used to drive predictive, environment-aware routing behavior while remaining compatible with adversarially robust bandit backbones.
...
\begin{enumerate}
\item \textbf{Algorithm Performance Ranking}: Which iCMAB algorithms (CPursuit, CEpsilonGreedy, CThompsonSampling, CEXP4, CEpochGreedy) demonstrate superior performance across realistic quantum network conditions?
...
\item \textbf{Hybrid Integration Viability}: Which iCMAB baselines provide the most promising anchors for future hybrid integration with adversarially robust models (e.g., CExpNeuralUCB) in our broader quantum routing framework?
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Multi-Scenario iCMAB Benchmarking}: Systematic evaluation of five iCMAB algorithms across five environment classes (NONE, STOCHASTIC, MARKOV, ADAPTIVE ATTACKS, ONLINE ADAPTIVE ATTACKS) over 3-run and 5-run experiment suites.
...
\item \textbf{Baseline Selection Framework}: Data-driven recommendations for which iCMAB algorithms should serve as baselines for hybrid CPursuit-/CEpsilonGreedy-anchored architectures and which should be avoided.
\end{enumerate}
...
The iCMAB algorithms evaluated in this report are:
...
\textbf{Observations:}
\begin{itemize}
\item CPursuit consistently leads the pack, with 3-run and 5-run averages differing by only 0.3 percentage points.
...
Aggregating across capacities, regimes, and run counts, CPursuit maintains the highest average efficiency in every environment:
...
\begin{table}[H]
\centering
\caption{Algorithm Selection Framework for iCMAB Baselines}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Deployment Scenario} & \textbf{Primary Choice} & \textbf{Secondary Choice} & \textbf{Rationale} \\
\hline
\hline
Maximum Efficiency + Stability & CPursuit & CEpsilonGreedy & Highest average efficiency, stable across scales \\
Predictive Layer for Hybrids   & CPursuit & CEpsilonGreedy & Strong baselines for CExpNeuralUCB+iCMAB \\
...
For future CExpNeuralUCB + iCMAB integration, the following design implications emerge:

\begin{itemize}
\item \textbf{Baseline Target}: New hybrids should match or exceed the CPursuit/CEpsilonGreedy band ($\gtrsim$88–90\% average efficiency) across environments and capacity scales.
...
\begin{itemize}
\item The present analysis focuses on the iCMAB subset (CPursuit, CEpsilonGreedy, CThompsonSampling, CEXP4, CEpochGreedy) and does not include updated log-backed results for neural or hybrid contextual models.
...
\item \textbf{Fairness and Equity Layers}: Integrate EQUITAS-style fairness constraints into iCMAB-based routing decisions to study equity-aware quantum resource allocation.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Clear Performance Hierarchy}: CPursuit and CEpsilonGreedy consistently form the top performance band across all environments, capacity scales, evaluator regimes, and run counts.
...
\item \textbf{Baseline Guidance for Hybrids}: The updated iCMAB evaluation provides a concrete, log-backed foundation for designing and benchmarking future hybrid CExpNeuralUCB + iCMAB architectures.
\end{enumerate}
...
\begin{itemize}
\item Use CPursuit and CEpsilonGreedy as default iCMAB baselines for new hybrid prototypes.
...
\begin{center}
\textbf{Interactive iCMAB Evaluation Notebooks (to be updated with final Colab links):}
\vspace{0.3cm}

\href{https://colab.research.google.com/drive/PLACEHOLDER_3_RUNS}{\colab{\texttt{iCMAB-Eval-3\_Run\_Suites}}}

\href{https://colab.research.google.com/drive/PLACEHOLDER_5_RUNS}{\colab{\texttt{iCMAB-Eval-5\_Run\_Suites}}}

\vspace{0.3cm}
\textbf{Capabilities:}
\begin{itemize}
\item 3-run and 5-run iCMAB evaluation suites with T-type and Tb-type regimes
...
\item Extensible hooks for hybrid CExpNeuralUCB + iCMAB experiments
\end{itemize}
...
This research was conducted as part of Graduate Assistant work in the AI/Quantum Computing track at Rochester Institute of Technology. The iCMAB-focused evaluation and the 3-run/5-run multi-capacity design provide a robust, log-backed empirical foundation for future hybrid algorithm development that integrates informed contextual multi-armed bandits with adversarially robust quantum routing architectures.
