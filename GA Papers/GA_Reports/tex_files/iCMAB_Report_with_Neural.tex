...
\item \legacy{Neural Baseline}: EXPNeuralUCB achieves 87.0\% stochastic efficiency, indicating modern neural approaches match informed contextual baselines while iCMAB strategies remain viable.
...
\item \textbf{Neural Baseline Comparison}: Legacy EXPNeuralUCB integration for context on neural vs. informed contextual approaches.
\end{itemize}
...
\item \textbf{Neural Baselines}: EXPNeuralUCB (Default allocator) for legacy comparison context.
\end{itemize}
...
\item \legacy{Neural Parity}: EXPNeuralUCB achieves 87.0\% stochastic efficiency, positioned at top tier with iCEpsilonGreedy-level performance but superior consistency (CV = 0.4\%).
\item \secondarytext{Tiered Structure}: Four-tier hierarchy: (1) EXPNeuralUCB + iCEpsilonGreedy (87.0\%, 86.8\%), (2) iCPursuit + iCThompsonSampling (68.5\%, 66.3\%), (3) iCEXP4 + iCEpochGreedy (37.1\%, 37.6\%).
...
\item \legacy{Neural Baseline}: EXPNeuralUCB achieves 86.3\% baseline, notably lower than iCEpsilonGreedy (93.2\%) but with outstanding consistency (CV = 0.1\%).
...
\item \legacy{EXPNeuralUCB Anomaly}: Shows negative degradation (-0.8\%), indicating stochastic performance (87.0\%) exceeds baseline (86.3\%), an unusual characteristic requiring further investigation.
\end{itemize}
...
\item \legacy{EXPNeuralUCB Stability}: Superior range of 0.8 pp with Std Dev 0.33 demonstrates exceptional consistency, surpassing iCEpsilonGreedy's variance profile.
...
\item \legacy{EXPNeuralUCB Consistency}: 0.1\% difference confirms exceptional stability across run-count variations.
\end{itemize}
...
\legacy{vs. EXPNeuralUCB} & 68.5\% & 87.0\% & +18.5\% & Neural Parity \\
\midrule
...
\item \warning{Neural Parity}: EXPNeuralUCB achieves 87.0\% stochastic, matching iCEpsilonGreedy (86.8\%), suggesting neural approaches provide competitive alternative.
...
Neural Baseline & Include EXPNeuralUCB for context & Evaluated for comparison ✓ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
\item \textbf{Tier 1 (Elite)}: iCEpsilonGreedy (86.8\% stochastic, CV = 2.3\%), EXPNeuralUCB (87.0\% stochastic, CV = 0.4\%) --- primary baselines for hybrid anchoring.
...
\midrule
Primary Hybrid Baseline (iCMAB) & iCEpsilonGreedy & 86.8\% efficiency, 2.3\% CV, excellent stability \\
Primary Hybrid Baseline (Neural) & EXPNeuralUCB & 87.0\% efficiency, 0.4\% CV, superior consistency \\
...
\item \textbf{Neural Alternative}: EXPNeuralUCB provides competitive tier-1 baseline with superior consistency if neural integration is preferred.
...
\item \textbf{Neural Investigation}: Investigate EXPNeuralUCB degradation anomaly and compare tier-1 baselines (iCEpsilonGreedy vs. EXPNeuralUCB) in hybrid context.
...
\item \secondarytext{Clear tiered hierarchy} established: Tier 1 iCEpsilonGreedy/EXPNeuralUCB, Tier 2 iCPursuit/iCThompsonSampling, Tier 3 iCEXP4/iCEpochGreedy.
...
\item \success{Neural Tier-1 Option}: EXPNeuralUCB provides viable tier-1 alternative if neural integration preferred, with superior consistency characteristics.
\end{enumerate}
...
\begin{enumerate}
\item Evaluate both iCEpsilonGreedy and EXPNeuralUCB variants in hybrid framework (parallel comparison).
\item Run convergence validation (3-run stochastic protocol) for both variants against current iCPursuit implementation.
\item Document performance delta and rationale for algorithm selection in hybrid paper.
\end{enumerate}

\textbf{Deployment Strategy:}
\begin{itemize}
\item \textbf{Primary Baseline (iCMAB Path)}: iCEpsilonGreedy (86.8\%) for optimal informed contextual routing.
\item \textbf{Primary Baseline (Neural Path)}: EXPNeuralUCB (87.0\%) if neural integration preferred, with superior consistency.
...
\item \textbf{Fallback}: If hybrid development encounters challenges, either iCEpsilonGreedy or EXPNeuralUCB alone provides competitive baseline.
\end{itemize}

\subsection{Broader Implications}

Informed contextual bandits and modern neural approaches provide high-performance, stable foundations for predictive quantum routing. The dominance and robustness of iCEpsilonGreedy, combined with competitive tier-1 performance from EXPNeuralUCB, establish clear guidance for the next phase: integrating these informed/neural models with adversarially robust bandits to achieve predictive, resilient quantum network routing under realistic and hostile conditions. The identified implementation gap represents both a discovery opportunity and a performance lever for near-term improvements.

\newpage
\section{Acknowledgments}

Prepared as part of Graduate Assistant work in AI/Quantum Computing at Rochester Institute of Technology. The comprehensive iCMAB evaluation documented here (December 11, 2025) provides a rigorous empirical foundation for iCMAB-EXPNeuralUCB hybrid development, complementing prior CMAB benchmarking and addressing critical algorithm selection decisions for quantum network routing under stochastic and adversarial conditions. The identification of the implementation gap and tier-1 neural alternative represent key findings that will accelerate hybrid optimization and deployment readiness.
