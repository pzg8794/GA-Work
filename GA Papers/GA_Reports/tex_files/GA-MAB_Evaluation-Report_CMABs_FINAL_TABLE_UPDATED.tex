...
\item \neural{Neural Enhancement}: CPursuitNeuralUCB (Default allocator) achieves \textbf{91.6\%} overall efficiency (with matching stochastic performance), demonstrating measurable improvement through neural integration.
...
\item \neural{Neural CMAB Variants}: GNeuralUCB (Default allocator) achieves \textbf{88.8\%} and EXPNeuralUCB (Default allocator) achieves \textbf{87.0\%}, establishing multiple competitive neural pathways.
...
\begin{enumerate}
\item \textbf{Algorithm Performance Ranking}: Which CMAB algorithms (CPursuit, CEpsilonGreedy, CThompsonSampling, CEXP4, CEpochGreedy) and neural variants demonstrate superior performance across realistic quantum network conditions?
...
\item \textbf{Hybrid Integration Viability}: Which CMAB baselines and neural variants provide the most promising anchors for future hybrid integration with adversarially robust models (e.g., CExpNeuralUCB) in our broader quantum routing framework?
\end{enumerate}
...
\item \textbf{Neural Variant Integration}: Comprehensive evaluation of CPursuitNeuralUCB (Default allocator), GNeuralUCB (Default allocator), and EXPNeuralUCB (Default allocator) variants to establish neural baselines for comparison with pure CMAB methods.
...
\item \textbf{Baseline Selection Framework}: Data-driven recommendations for which CMAB algorithms and neural variants should serve as baselines for hybrid CPursuit-/CEpsilonGreedy-anchored architectures and which should be avoided.
\end{enumerate}
...
\item \textbf{CPursuitNeuralUCB}: Neural-enhanced contextual pursuit with learned representations and confidence bounds (Default allocator); provides superior neural integration of pursuit-based methods.
\item \textbf{GNeuralUCB}: Neural contextual bandit using Gaussian process-style confidence bounds with neural feature representations (Default allocator); provides competitive neural alternative.
\item \textbf{EXPNeuralUCB}: Neural contextual bandit using exponential-weight style learning (Default allocator); legacy neural baseline.
\end{itemize}

These contextual baselines are intended to provide a predictive layer that can be later combined with adversarially robust models such as EXPNeuralUCB and its hybrid variants.
...
\begin{table}[h!]
\centering
\caption{Global CMAB and Neural Performance Across 3-run and 5-run Suites (Default allocator for neural baselines)}
\label{tab:global-performance}
\begin{tabular}{lccc}
\toprule
\textbf{Algorithm} & \textbf{3-run Avg Eff. (\%)} & \textbf{5-run Avg Eff. (\%)} & \textbf{Overall Avg Eff. (\%)} \\
\midrule
CPursuitNeuralUCB & 92.2 & 92.4 & \textbf{92.3} \\
GNeuralUCB        & 88.5 & 86.9 & \textbf{87.7} \\
EXPNeuralUCB      & 84.3 & 85.7 & \textbf{85.0} \\
\bottomrule
\end{tabular}
\end{table}
...
\legacy{EXPNeuralUCB (Default)}       & \legacy{86.4}     & \legacy{87.4}     & \legacy{87.0} \\
\hline
\end{tabular}
\end{table}
...
\textbf{Observations:}
\begin{itemize}
\item \neural{CPursuitNeuralUCB (Default allocator) achieves 91.6\% overall efficiency}, leading all algorithms with excellent convergence between 3-run (91.1\%) and 5-run (91.8\%) suites.
...
\item \neural{GNeuralUCB (Default allocator) achieves 88.8\% overall}, positioned between CPursuit (90.4\%) and CEpsilonGreedy (87.8\%) among pure contextual methods.
...
\item \neural{CPursuitNeuralUCB integration shows 1.2 pp improvement} over traditional CPursuit, demonstrating significant value of neural enhancement.
\item \neural{GNeuralUCB integration shows 1.0 pp improvement} over CEpsilonGreedy, establishing Gaussian-style neural integration as viable.
\item \legacy{EXPNeuralUCB (Default allocator)} achieves 87.0\% overall, positioned between CEpsilonGreedy and the mid-range algorithms.
...
The STOCHASTIC condition models natural random link failures and decoherence without an intelligent adversary. Table~\ref{tab:stoch-performance} reports the average efficiency per algorithm in the stochastic environment (aggregated over capacity scales, evaluator regimes, and run counts). Neural entries are again for the \textbf{Default allocator} only.

\begin{table}[H]
\small
\centering
\caption{\stochastic{Stochastic Environment Performance Summary (Default allocator for neural baselines)}}
...
\legacy{EXPNeuralUCB (Default)}        & \legacy{87.0}               & \legacy{13.0}     & \legacy{9.7}     & \legacy{Moderate} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
\item \neural{CPursuitNeuralUCB (Default allocator) dominates}: achieving the highest efficiency in stochastic ($\approx$91.6\%) with excellent coefficient of variation ($\approx$3.2\%).
...
\item \neural{GNeuralUCB (Default allocator) Competitive}: achieves 88.8\% stochastic efficiency with exceptional consistency (CV = 1.3\%), forming a strong neural alternative tier.
\item \stochastic{Strong Tradition Tier}: CEpsilonGreedy remains close to CPursuit (88.7\%), confirming its role as a strong secondary baseline.
...
\item \legacy{Legacy Baseline}: EXPNeuralUCB (Default allocator) achieves 87.0\% in stochastic conditions with substantially higher variance (9.7\% CV), indicating less stable performance than top methods.
\end{itemize}
...
\legacy{EXPNeuralUCB (Default)}        & \legacy{86.6}     & \legacy{88.3}     & \legacy{85.9} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
\item \neural{CPursuitNeuralUCB (Default allocator) shows consistent performance}, with peak at 1.5$\times$ (92.6\%), demonstrating effective scaling properties.
\item \neural{GNeuralUCB (Default allocator) demonstrates stable scaling} across capacity regimes with minimal variance, confirming Gaussian-style robustness.
...
\item \legacy{EXPNeuralUCB (Default allocator) shows higher sensitivity across scales}, with a 2.4 pp gap between 1.5$\times$ and 2$\times$.
\end{itemize}

\newpage
\subsection{Evaluator Regime Comparison: T-type vs. Tb-type}

Table~\ref{tab:regime-comparison} compares average efficiencies for T-type and Tb-type runs aggregated across capacities, environments, and run counts. Neural baselines are again measured under the Default allocator.
...
\legacy{EXPNeuralUCB (Default)}        & \legacy{86.5}     & \legacy{87.4}     & \legacy{+0.9} \\
\hline
\end{tabular}
\end{table}
...
\textbf{Interpretation:}
\begin{itemize}
\item \neural{CPursuitNeuralUCB (Default allocator) is robust across evaluator regimes}, with consistent performance (91.4\% T-type, 91.7\% Tb-type).
\item \neural{GNeuralUCB (Default allocator) shows balanced regime performance}, with +0.6 pp Tb-type advantage, similar to CEpsilonGreedy pattern.
...
\item \legacy{EXPNeuralUCB (Default allocator) shows modest Tb-type advantage (+0.9 pp)}, similar in magnitude to top-tier methods.
\end{itemize}
...
\item \neural{GNeuralUCB (Default allocator) provides a competitive neural tier across all environments}.
\item CThompsonSampling and CEXP4 provide mid-range baselines that react to environment changes but do not surpass CPursuit/CEpsilonGreedy/GNeuralUCB.
...
\hline
\hline
Maximum Efficiency Overall & CPursuitNeuralUCB (Default) & CPursuit & 91.6\% efficiency, neural advantage \\
Maximum Efficiency (Pure CMAB) & CPursuit & CEpsilonGreedy & Highest traditional CMAB efficiency \\
Neural Gaussian Alternative & GNeuralUCB (Default) & EXPNeuralUCB (Default) & Competitive neural pathway \\
...
\begin{itemize}
\item \textbf{Baseline Target (Pure)}: New hybrids should match or exceed CPursuit/CEpsilonGreedy band ($\gtrsim$88–90\% average efficiency) across environments and capacity scales.
\item \textbf{Baseline Target (Neural)}: New hybrids should match or exceed CPursuitNeuralUCB (Default, 91.6\%) or GNeuralUCB (Default, 88.8\%) across environments and capacity scales.
...
\item \textbf{Neural Enhancement Potential}: CPursuitNeuralUCB's 1.2 pp gain and GNeuralUCB's 1.0 pp gain over pure methods confirm neural integration value for contextual bandit routing decisions.
\end{itemize}
...
\begin{itemize}
\item The present analysis focuses on the CMAB subset (CPursuit, CEpsilonGreedy, CThompsonSampling, CEXP4, CEpochGreedy) with comprehensive neural variant assessment under the Default allocator.
...
\begin{enumerate}
\item \textbf{Hybrid Integration}: Implement and evaluate CExpNeuralUCB variants that plug CPursuit/CEpsilonGreedy-based or neural variant predictive layers into adversarially robust bandit backbones.
...
\item \textbf{Neural Architecture Comparison}: Characterize performance differences between CPursuitNeuralUCB (Default), GNeuralUCB (Default), and EXPNeuralUCB (Default) architectures.
\item \textbf{Fairness and Equity Layers}: Integrate EQUITAS-style fairness constraints into CMAB and neural-based routing decisions to study equity-aware quantum resource allocation.
\end{enumerate}
...
\item \textbf{Neural Variant Tracking}: CPursuitNeuralUCB, GNeuralUCB, and EXPNeuralUCB are explicitly tracked and reported here under the Default allocator configuration.
\end{itemize}
...
\begin{enumerate}
\item \neural{CPursuitNeuralUCB (Default allocator) Leadership}: achieves 91.6\% overall efficiency, establishing neural contextual integration as the leading approach.
\item \textbf{Clear Traditional CMAB Hierarchy}: CPursuit and CEpsilonGreedy consistently form the top traditional CMAB performance band across all environments, capacity scales, evaluator regimes, and run counts.
\item \neural{Competitive Neural Alternatives}: GNeuralUCB (Default allocator) achieves 88.8\% overall and EXPNeuralUCB (Default allocator) achieves 87.0\%, establishing multiple viable neural pathways.
...
\item \neural{Neural Enhancement Validation}: CPursuitNeuralUCB's 1.2 pp advantage and GNeuralUCB's 1.0 pp advantage over pure methods confirm neural integration value for contextual bandit routing.
\item \legacy{Legacy Comparison}: EXPNeuralUCB (Default allocator) achieves 87.0\% stochastic efficiency, confirming that modern contextual anchoring improves neural approaches.
\end{enumerate}
...
\begin{itemize}
\item Use CPursuitNeuralUCB (Default allocator) as the primary contextual baseline for new hybrid prototypes (91.6\% overall).
\item CPursuit as reliable secondary pure-CMAB choice (90.4\% overall).
\item GNeuralUCB (Default allocator) as tertiary neural alternative with excellent consistency (88.8\% overall).
...
\item Investigate why CPursuitNeuralUCB (Default allocator) outperforms other neural variants and explore hybrid neural architectures combining best properties of multiple approaches.
\end{itemize}

\begin{tcolorbox}[colback=colabcolor!20,colframe=colabcolor,title=\textbf{Interactive Framework Access (Placeholder Links)}]

\begin{center}
\textbf{Interactive CMAB and Neural Evaluation Notebooks (to be updated with final Colab links):}
\vspace{0.3cm}

\href{https://colab.research.google.com/drive/PLACEHOLDER_3_RUNS}{\colab{\texttt{CMAB-Neural-Eval-3\_Run\_Suites}}}

\href{https://colab.research.google.com/drive/PLACEHOLDER_5_RUNS}{\colab{\texttt{CMAB-Neural-Eval-5\_Run\_Suites}}}

\vspace{0.3cm}
\textbf{Capabilities:}
\begin{itemize}
\item 3-run and 5-run CMAB and neural variant evaluation suites with T-type and Tb-type regimes
...
\item Neural variant comparison and selection guidance
\item Extensible hooks for hybrid CExpNeuralUCB + CMAB/Neural experiments
\end{itemize}
...
This research was conducted as part of Graduate Assistant work in the AI/Quantum Computing track at Rochester Institute of Technology. The CMAB-focused evaluation integrated with comprehensive neural variant assessment (CPursuitNeuralUCB, GNeuralUCB, EXPNeuralUCB; all under the Default allocator) and the 3-run/5-run multi-capacity design provide a robust, log-backed empirical foundation for future hybrid algorithm development that integrates contextual multi-armed bandits with neural integration and adversarially robust quantum routing architectures. The confirmation of neural enhancement's effect, particularly CPursuitNeuralUCB's leadership and GNeuralUCB's stability at 88.8\%, establishes a strategic pathway for quantum network routing optimization.
