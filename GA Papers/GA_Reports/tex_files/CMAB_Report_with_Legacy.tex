...
\item \legacy{Legacy Baseline}: EXPNeuralUCB achieves \textbf{87.0\%} stochastic efficiency with 9.7\% CV, confirming modern CMAB approaches outperform legacy neural methods.
...
\item \legacy{Legacy Baseline}: EXPNeuralUCB achieves 87.0\% in stochastic conditions with substantially higher variance (9.7\% CV), indicating less stable performance than top CMAB methods.
\end{itemize}
...
\item \legacy{EXPNeuralUCB shows modest Tb-type advantage (+0.9 pp)}, similar in magnitude to top-tier CMAB methods.
\end{itemize}
...
\begin{itemize}
\item The present analysis focuses on the CMAB subset (CPursuit, CEpsilonGreedy, CThompsonSampling, CEXP4, CEpochGreedy) and does not include updated log-backed results for neural or hybrid contextual models.
...
\item \legacy{Legacy Comparison}: EXPNeuralUCB achieves 87.0\% stochastic efficiency, confirming that modern CMAB methods provide improved performance and consistency over legacy neural approaches.
\end{enumerate}
...
\begin{itemize}
\item Use CPursuit and CEpsilonGreedy as default CMAB baselines for new hybrid prototypes.
