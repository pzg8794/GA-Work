...
\begin{itemize}
\item \neural{Top Neural Performer}: iCPursuitNeuralUCB achieves 88.9\% Oracle efficiency (stochastic) with superior baseline performance of 91.1\%, excellent variance (CV = 1.8\%), demonstrating strength of neural integration of informed pursuit methods.
\item \primarytext{Top Informed Performer}: iCEpsilonGreedy achieves 86.8\% Oracle efficiency (stochastic) with exceptional baseline performance of 93.2\%, lowest variance (CV = 2.3\%) among pure iCMAB algorithms.
\item \secondarytext{Performance Tiers}: A clear hierarchy emerges: Tier 1 -- iCPursuitNeuralUCB (88.9\% neural), iCEpsilonGreedy (86.8\% pure); Tier 2 -- iCPursuit (68.5\%) and iCThompsonSampling (66.3\%); Tier 3 -- iCEXP4 (37.1\%) and iCEpochGreedy (37.6\%).
\item \legacy{Neural Alternatives}: EXPNeuralUCB achieves 87.0\% stochastic efficiency, indicating multiple neural pathways provide competitive tier-1 alternatives.
...
\item \success{Capacity and Regime Stability}: Across scaled capacities (1$\times$--2$\times$) and different evaluator types, iCEpsilonGreedy and iCPursuitNeuralUCB maintain consistent ranking with variance $<$2.1\%, confirming dominance is fundamental rather than configuration-dependent.
...
\item \primarytext{Hybrid Integration Ready}: iCEpsilonGreedy (Tier 1 pure) and iCPursuitNeuralUCB (Tier 1 neural) identified as primary candidates for iCMAB-EXPNeuralUCB hybrids, with explicit performance targets of $\geq$86.8\% for pure and $\geq$88.9\% for neural variants.
\item \warning{Critical Finding}: Prior hybrid development used iCPursuit (CMAB winner) without testing iCEpsilonGreedy (iCMAB winner) or neural variants. Switching to iCPursuitNeuralUCB offers immediate 20.4\% performance gain (68.5\% → 88.9\%).
\end{itemize}
\end{tcolorbox}
...
\item \textbf{Neural Integration Analysis}: Comprehensive evaluation of iCPursuitNeuralUCB demonstrating superior neural integration of informed pursuit methods.
...
\item \textbf{Neural Baselines}: iCPursuitNeuralUCB and EXPNeuralUCB (both Default allocator) for comprehensive neural comparison.
\end{itemize}
...
\item \neural{Neural Enhancement}: iCPursuitNeuralUCB outperforms its pure iCPursuit counterpart by 20.4 percentage points (68.5\% → 88.8\%), demonstrating substantial neural integration value.
...
\item \primarytext{Tier 1 Excellence}: iCEpsilonGreedy's CV of 2.3\% indicates exceptional consistency across runs among pure algorithms.
\item \legacy{Neural Parity}: EXPNeuralUCB achieves 87.0\% stochastic efficiency, positioned at tier-1 level with iCEpsilonGreely but superior consistency (CV = 0.4\%).
\item \secondarytext{Tiered Structure}: Four-tier hierarchy: (1) iCPursuitNeuralUCB (88.9\% neural) + iCEpsilonGreedy (86.8\% pure) + EXPNeuralUCB (87.0\% neural); (2) iCPursuit + iCThompsonSampling (68.5\%, 66.3\%); (3) iCEXP4 + iCEpochGreedy (37.1\%, 37.6\%).
...
\item \primarytext{Exceptional Consistency}: iCEpsilonGreedy baseline CV of 0.4\% demonstrates remarkable stability, matching iCPursuitNeuralUCB stability (0.3\%).
\item \legacy{Neural Baseline}: EXPNeuralUCB achieves 86.3\% baseline, notably lower than top pure methods but with outstanding consistency (CV = 0.1\%).
...
\item \warning{Top-Tier Degradation}: Both iCEpsilonGreedy (6.4\%) and iCPursuitNeuralUCB (2.2\%) show substantial degradation, but iCPursuitNeuralUCB maintains superior absolute stochastic performance.
\end{itemize}
...
\item \success{Neural Advantage}: iCPursuitNeuralUCB's 2.2\% absolute gap significantly outperforms iCEpsilonGreedy's 6.4\% gap while maintaining higher baseline, representing superior robustness profile.
...
\item \legacy{EXPNeuralUCB Stability}: Superior range of 0.8 pp with Std Dev 0.33 demonstrates exceptional consistency, best among neural alternatives.
...
\item \legacy{EXPNeuralUCB Stability}: Superior 0.3\% max variance across scales confirms exceptional robustness independent of capacity regime.
...
\item \neural{Stability Improvement (Neural)}: 6.5$\times$ better stability (CV: 6.5\% → 1.0\%) makes iCPursuitNeuralUCB extraordinarily predictable for deployment.
...
Neural Evaluation & Test neural variants of top methods & iCPursuitNeuralUCB evaluated (Dec 9) ✓ \\
Hybrid Dev & Use best iCMAB or neural variant & Used CMAB assumption ✗ ✗ \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Tier 1 (Elite Pure)}: iCEpsilonGreedy (86.8\% stochastic, CV = 2.3\%), EXPNeuralUCB (87.0\% stochastic, CV = 0.4\%) --- premier pure/alternative neural options.
...
Neural Alternative & EXPNeuralUCB & 87.0\% efficiency, 0.4\% CV, superior consistency \\
...
\item \textbf{Neural First Strategy}: iCPursuitNeuralUCB recommended as primary baseline due to superior performance, stability, and degradation resilience.
...
\item \textbf{Hybrid Benchmarking}: Evaluate iCPursuitNeuralUCB+EXPNeuralUCB vs. iCEpsilonGreedy+EXPNeuralUCB vs. iCPursuit+EXPNeuralUCB to quantify actual hybrid performance gains.
...
\begin{enumerate}
\item \neural{iCPursuitNeuralUCB establishes new leadership} with 88.9\% stochastic efficiency, 91.1\% baseline performance, and exceptional consistency (CV = 1.0\%), demonstrating neural integration transforms iCPursuit performance.
...
\item \legacy{EXPNeuralUCB matches tier-1 performance} with 87.0\% stochastic efficiency and superior consistency (CV = 0.4\%), establishing neural approaches as competitive alternative.
\item \secondarytext{Clear tiered hierarchy} established: Tier 1 iCPursuitNeuralUCB (88.9\% neural), iCEpsilonGreedy (86.8\% pure), EXPNeuralUCB (87.0\% neural); Tier 2 iCPursuit/iCThompsonSampling; Tier 3 iCEXP4/iCEpochGreedy.
\item \success{Neural Enhancement Dramatic}: iCPursuitNeuralUCB achieves 20.4\% improvement over pure iCPursuit (68.5\% → 88.9\%), demonstrating exceptional neural integration value for pursuit-based methods.
...
\item \success{Degradation Resilience}: iCPursuitNeuralUCB's 2.2\% baseline-to-stochastic gap significantly outperforms iCEpsilonGreedy's 6.4\% gap, solving robustness trade-off.
\end{enumerate}
...
\item Document 20.4\% performance delta and neural integration strategy in hybrid paper.
\item Prepare fallback evaluation of iCEpsilonGreedy + EXPNeuralUCB if neural integration encounters technical challenges.
\end{enumerate}
...
\item \textbf{Secondary Baseline (Neural)}: EXPNeuralUCB (87.0\%) with superior consistency if alternative neural path needed.
