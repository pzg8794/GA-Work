...
\item \neural{Neural Enhancement}: CPursuitNeuralUCB (Default allocator) achieves \textbf{91.6\%} overall efficiency, demonstrating 1.2 pp improvement through neural integration.
...
\item \neural{Neural CMAB Variants}: GNeuralUCB achieves \textbf{88.9\%} and EXPNeuralUCB achieves \textbf{87.0\%}, establishing multiple competitive neural pathways.
...
\item \textbf{Neural Variant Integration}: Comprehensive evaluation of CPursuitNeuralUCB, GNeuralUCB, and EXPNeuralUCB variants to establish neural baselines for comparison with pure CMAB methods.
...
\item \textbf{CPursuitNeuralUCB}: Neural-enhanced contextual pursuit with learned representations; provides superior neural integration of pursuit-based methods.
...
\item \textbf{EXPNeuralUCB}: Neural contextual bandit using exponential-weight style learning; legacy neural baseline.
\end{itemize}
...
\item \neural{GNeuralUCB achieves 88.9\% overall}, positioned between CPursuit (90.4\%) and CEpsilonGreedy (87.8\%) among pure contextual methods.
...
\item \neural{GNeuralUCB integration shows 1.1 pp improvement} over CEpsilonGreedy, establishing Gaussian-style neural integration as viable.
...
\item \neural{GNeuralUCB Competitive}: GNeuralUCB achieves 88.9\% stochastic efficiency with exceptional consistency (CV = 1.3\%), forming a strong neural alternative tier.
...
\item \neural{GNeuralUCB demonstrates stable scaling} across capacity regimes with minimal variance, confirming Gaussian-style robustness.
...
\item \neural{GNeuralUCB shows balanced regime performance}, with +0.6 pp Tb-type advantage, similar to CEpsilonGreedy pattern.
...
\item \neural{GNeuralUCB provides a competitive neural tier across all environments}.
...
\hline
\hline
Maximum Efficiency Overall & CPursuitNeuralUCB & CPursuit & 91.6\% efficiency, neural advantage \\
Maximum Efficiency (Pure CMAB) & CPursuit & CEpsilonGreedy & Highest traditional CMAB efficiency \\
Neural Gaussian Alternative & GNeuralUCB & EXPNeuralUCB & Competitive neural pathway \\
...
\item \textbf{Baseline Target (Neural)}: New hybrids should match or exceed CPursuitNeuralUCB (91.6\%) or GNeuralUCB (88.9\%) across environments and capacity scales.
...
\item \textbf{Neural Enhancement Potential}: CPursuitNeuralUCB's 1.2 pp gain and GNeuralUCB's 1.1 pp gain demonstrate room for neural improvement in contextual routing decisions.
\end{itemize}
...
\begin{itemize}
\item The present analysis focuses on the CMAB subset (CPursuit, CEpsilonGreedy, CThompsonSampling, CEXP4, CEpochGreedy) with comprehensive neural variant assessment.
...
\item \textbf{Neural Architecture Comparison}: Characterize performance differences between CPursuitNeuralUCB, GNeuralUCB, and EXPNeuralUCB architectures.
...
\item \textbf{Neural Variant Tracking}: CPursuitNeuralUCB, GNeuralUCB, and EXPNeuralUCB tracked with allocator specifications (Default allocator only reported here).
\end{itemize}
...
\begin{enumerate}
\item \neural{CPursuitNeuralUCB Leadership}: CPursuitNeuralUCB (Default allocator) achieves 91.6\% overall efficiency, establishing neural contextual integration as the leading approach.
...
\item \neural{Competitive Neural Alternatives}: GNeuralUCB achieves 88.9\% overall and EXPNeuralUCB achieves 87.0\%, establishing multiple viable neural pathways.
...
\item \neural{Neural Enhancement Validation}: CPursuitNeuralUCB's 1.2 pp advantage and GNeuralUCB's 1.1 pp advantage over pure methods confirm neural integration value for contextual bandit routing.
...
\begin{itemize}
\item Use CPursuitNeuralUCB as the primary contextual baseline for new hybrid prototypes (91.6\% overall).
\item CPursuit as reliable secondary pure-CMAB choice (90.4\% overall).
\item GNeuralUCB as tertiary neural alternative with excellent consistency.
...
\item Investigate why CPursuitNeuralUCB outperforms other neural variants and explore hybrid neural architectures combining best properties of multiple approaches.
\end{itemize}
...
This research was conducted as part of Graduate Assistant work in the AI/Quantum Computing track at Rochester Institute of Technology. The CMAB-focused evaluation integrated with comprehensive neural variant assessment (CPursuitNeuralUCB, GNeuralUCB, EXPNeuralUCB) and the 3-run/5-run multi-capacity design provide a robust, log-backed empirical foundation for future hybrid algorithm development that integrates contextual multi-armed bandits with neural integration and adversarially robust quantum routing architectures. The discovery of neural enhancement's transformative effect, particularly CPursuitNeuralUCB's leadership, establishes a strategic pathway for quantum network routing optimization.
